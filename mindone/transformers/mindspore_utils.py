...
ALL_LAYERNORM_LAYERS = [mint.nn.LayerNorm]
...
def prune_linear_layer(layer: mint.nn.Linear, index: ms.Tensor, dim: int = 0) -> mint.nn.Linear:
    ...
    Args:
        layer (`mindspore.mint.nn.Linear`): The layer to prune.
...
def prune_layer(layer: Union[mint.nn.Linear, Conv1D], index: ms.Tensor, dim: Optional[int] = None) -> Union[nn.Dense, Conv1D]:
    """
    ...
    Args:
        layer (`Union[mint.nn.Linear, Conv1D]`): The layer to prune.
        index (`torch.LongTensor`): The indices to keep in the layer.
        dim (`int`, *optional*): The dimension on which to keep the indices.

    Returns:
        `mint.nn.Linear` or [`~pytorch_utils.Conv1D`]: The pruned layer as a new layer with `requires_grad=True`.
    """
    if isinstance(layer, mint.nn.Linear):
