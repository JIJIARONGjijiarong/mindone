...
class BertLMPredictionHead(nn.Cell):
    def __init__(self, config):
        ...
        self.decoder = mint.nn.Linear(config.hidden_size, config.vocab_size, has_bias=False)
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, **model_kwargs):
        ...
        attention_mask = mint.cat([attention_mask, attention_mask.new_zeros((attention_mask.shape[0], 1))], axis=-1)
...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dtype=None):
    ...
    if attn_mask is not None:
        ...
        attn_weight = mint.nn.softmax(
            ops.cast(mint.matmul(query, key.swapaxes(-2, -1)) / (query.shape[-1] ** 0.5) + attn_mask, ms.float32),
            axis=-1,
        ).astype(_dtype)
    else:
        attn_weight = mint.nn.softmax(
            ops.cast(mint.matmul(query, key.swapaxes(-2, -1)) / (query.shape[-1] ** 0.5), ms.float32), axis=-1
        ).astype(_dtype)
