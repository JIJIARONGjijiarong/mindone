...
import numpy as np
from transformers.models.bit.configuration_bit import BitConfig
...
class BitStage(nn.Cell):
    ...
    def __init__(
        ...
        for layer_idx in range(depth):
            ...
            self.layers.insert_child_to_cell(
                str(layer_idx),
                layer_cls(
                    config,
                    prev_chs,
...
class BitEncoder(nn.Cell):
    def __init__(self, config: BitConfig):
        ...
        layer_dropouts = [
            x.tolist()
            for x in ms.Tensor(np.linspace(0, config.drop_path_rate, sum(config.depths))).split(config.depths)
        ]
        ...
        for stage_idx, (current_depth, current_hidden_size, layer_dropout) in enumerate(
            ...
            self.stages.insert_child_to_cell(str(stage_idx), stage)
    ...
    def construct(
        self, hidden_state: ms.Tensor, output_hidden_states: bool = False, return_dict: bool = True
...
class BitPreTrainedModel(MSPreTrainedModel):
    """
...
class BitModel(BitPreTrainedModel):
    ...
    def construct(
        self, pixel_values: ms.Tensor, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = False
...
class BitBackbone(BitPreTrainedModel, BackboneMixin):
    ...
    def construct(
        ...
        >>> from mindone.transformers import BitBackbone
        >>> import mindspore as ms
        ...
        >>> model = BitBackbone.from_pretrained("google/resnetnv2-50")
        ...
        outputs = self.bit(pixel_values, output_hidden_states=True, return_dict=False)
