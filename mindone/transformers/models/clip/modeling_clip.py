...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.visual_projection = mint.nn.Linear(self.vision_embed_dim, self.projection_dim, has_bias=False)
        self.text_projection = mint.nn.Linear(self.text_embed_dim, self.projection_dim, has_bias=False)
    ...
    def construct(
        ...
        >>> probs = mint.nn.Softmax(logits_per_image, dim=1)  # we can take the softmax to get the label probabilities
        ```"""
        # Use CLIP model's config for some fields (if specified) instead of those of vision & text components.
        output_attentions = output_attentions if output_attentions is not None else self.output_attentions
        output_hidden_states = output_hidden_states if output_hidden_states is not None else self.output_hidden_states
...
class CLIPTextModelWithProjection(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPTextConfig):
        ...
        self.text_projection = mint.nn.Linear(config.hidden_size, config.projection_dim, has_bias=False)
