...
class DabDetrDecoderOutput(BaseModelOutputWithCrossAttentions):
    ...
    intermediate_hidden_states (`mindspore.Tensor` of shape `(config.decoder_layers, batch_size, num_queries, hidden_size)`, *optional*, returned when `config.auxiliary_loss=True`):
        ...
        layernorm.
    reference_points (`mindspore.Tensor` of shape `(config.decoder_layers, batch_size, num_queries, 2 (anchor points))`):
        Reference points (reference points of each layer of the decoder).
    """

    intermediate_hidden_states: Optional[mindspore.Tensor] = None
    reference_points: Optional[tuple[mindspore.Tensor]] = None
...
class DabDetrObjectDetectionOutput(ModelOutput):
    ...
    pred_boxes: Optional[mindspore.Tensor] = None
...
class DabDetrFrozenBatchNorm2d(mindspore.nn.Cell):
    """
    ...
    def __init__(self, n):
        super().__init__()
        self.register_buffer("weight", mindspore.mint.ones(n))
        self.register_buffer("bias", mindspore.mint.zeros(n))
        self.register_buffer("running_mean", mindspore.mint.zeros(n))
        self.register_buffer("running_var", mindspore.mint.ones(n))
...
def replace_batch_norm(model):
    r"""
    Recursively replace all `mindspore.nn.BatchNorm2d` with `DabDetrFrozenBatchNorm2d`.
    ...
    for name, module in model.named_children():
        if isinstance(module, mindspore.mint.nn.BatchNorm2d):
...
class DabDetrConvEncoder(mindspore.nn.Cell):
    """
    ...
    def construct(self, pixel_values: mindspore.Tensor, pixel_mask: mindspore.Tensor):
        ...
        for feature_map in features:
            ...
            mask = mindspore.mint.nn.functional.interpolate(pixel_mask[None].float(), size=feature_map.shape[-2:]).to(mindspore.bool_)[0]
            out.append((feature_map, mask))
        return out


# Copied from transformers.models.detr.modeling_detr.DetrConvModel with Detr->DabDetr
class DabDetrConvModel(mindspore.nn.Cell):
    """
...
class DabDetrSinePositionEmbedding(mindspore.nn.Cell):
    """
    ...
    def construct(self, pixel_values, pixel_mask):
        ...
        dim_tx = mindspore.mint.arange(self.embedding_dim, dtype=mindspore.float32, )
        ...
        # We use float32 to ensure reproducibility of the original implementation
        dim_ty = mindspore.mint.arange(self.embedding_dim, dtype=mindspore.float32, )
        ...
        pos_x = mindspore.mint.stack((pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), dim=4).flatten(3)
        pos_y = mindspore.mint.stack((pos_y[:, :, :, 0::2].sin(), pos_y[:, :, :, 1::2].cos()), dim=4).flatten(3)
        pos = mindspore.mint.cat((pos_y, pos_x), dim=3).permute(0, 3, 1, 2)
        return pos
...
def gen_sine_position_embeddings(pos_tensor, hidden_size=256):
    ...
    scale = 2 * math.pi
    dim = hidden_size // 2
    dim_t = mindspore.mint.arange(dim, dtype=mindspore.float32, )
    dim_t = 10000 ** (2 * mindspore.mint.div(dim_t, 2, rounding_mode="floor") / dim)
    x_embed = pos_tensor[:, :, 0] * scale
    y_embed = pos_tensor[:, :, 1] * scale
    pos_x = x_embed[:, :, None] / dim_t
    pos_y = y_embed[:, :, None] / dim_t
    pos_x = mindspore.mint.stack((pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=3).flatten(2)
    pos_y = mindspore.mint.stack((pos_y[:, :, 0::2].sin(), pos_y[:, :, 1::2].cos()), dim=3).flatten(2)
    if pos_tensor.shape[-1] == 4:
        w_embed = pos_tensor[:, :, 2] * scale
        pos_w = w_embed[:, :, None] / dim_t
        pos_w = mindspore.mint.stack((pos_w[:, :, 0::2].sin(), pos_w[:, :, 1::2].cos()), dim=3).flatten(2)

        h_embed = pos_tensor[:, :, 3] * scale
        pos_h = h_embed[:, :, None] / dim_t
        pos_h = mindspore.mint.stack((pos_h[:, :, 0::2].sin(), pos_h[:, :, 1::2].cos()), dim=3).flatten(2)

        pos = mindspore.mint.cat((pos_y, pos_x, pos_w, pos_h), dim=2)
    else:
...
def inverse_sigmoid(x, eps=1e-5):
    ...
    return mindspore.mint.log(x1 / x2)


# Modified from transformers.models.detr.modeling_detr.DetrAttention
class DetrAttention(mindspore.nn.Cell):
    """
    ...
    def __init__(
        ...
        self.k_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=bias)
        self.v_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=bias)
        self.q_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=bias)
        self.out_proj = mindspore.mint.nn.Linear(self.hidden_size, self.hidden_size, bias=bias)
    ...
    def construct(
        ...
        batch_size, q_len, embed_dim = hidden_states.shape
        ...
        attn_weights = mindspore.mint.matmul(query_states, key_states.transpose(2, 3))
...
class DabDetrAttention(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: DabDetrConfig, bias: bool = True, is_cross: bool = False):
        ...
        self.output_proj = mindspore.mint.nn.Linear(self.output_dim, self.output_dim, bias=bias)

    def construct(
        self,
        hidden_states: mindspore.Tensor,
        attention_mask: Optional[mindspore.Tensor] = None,
        key_states: Optional[mindspore.Tensor] = None,
        value_states: Optional[mindspore.Tensor] = None,
        ...
        attn_weights = mindspore.mint.nn.functional.softmax(attn_weights, dim=-1, dtype=mindspore.float32).to(query_states.dtype)
        attn_probs = mindspore.mint.nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_output = mindspore.mint.matmul(attn_probs, value_states)

        if attn_output.shape != (batch_size, self.attention_heads, q_len, self.values_head_dim):
            raise ValueError(
...
class DabDetrDecoderLayerSelfAttention(mindspore.nn.Cell):
    def __init__(self, config: DabDetrConfig):
        super().__init__()
        self.dropout = config.dropout
        self.self_attn_query_content_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.self_attn_query_pos_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.self_attn_key_content_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.self_attn_key_pos_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.self_attn_value_proj = mindspore.mint.nn.Linear(config.hidden_size, config.hidden_size)
        self.self_attn = DabDetrAttention(config)
        self.self_attn_layer_norm = mindspore.mint.nn.LayerNorm(config.hidden_size)
...
class DabDetrDecoderLayerCrossAttention(mindspore.nn.Cell):
    def __init__(self, config: DabDetrConfig, is_first: bool = False):
        super().__init__()
        hidden_size = config.hidden_size
        self.cross_attn_query_content_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.cross_attn_query_pos_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.cross_attn_key_content_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.cross_attn_key_pos_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.cross_attn_value_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.cross_attn_query_pos_sine_proj = mindspore.mint.nn.Linear(hidden_size, hidden_size)
        self.decoder_attention_heads = config.decoder_attention_heads
        self.cross_attn_layer_norm = mindspore.mint.nn.LayerNorm(hidden_size)
    ...
    def construct(
        ...
        query_sine_embed: Optional[mindspore.Tensor] = None,
        output_attentions: Optional[bool] = None,
    ):
        ...
        query = mindspore.mint.cat([query, query_sine_embed], dim=3).view(batch_size, num_queries, n_model * 2)
        ...
        key = mindspore.mint.cat([key, key_pos], dim=3).view(batch_size, height_width, n_model * 2)
...
class DabDetrDecoderLayerFFN(mindspore.nn.Cell):
    def __init__(self, config: DabDetrConfig):
        super().__init__()
        hidden_size = config.hidden_size
        self.final_layer_norm = mindspore.mint.nn.LayerNorm(hidden_size)
        self.fc1 = mindspore.mint.nn.Linear(hidden_size, config.decoder_ffn_dim)
        self.fc2 = mindspore.mint.nn.Linear(config.decoder_ffn_dim, hidden_size)
...
class DabDetrEncoderLayer(nn.Cell):
    def __init__(self, config: DabDetrConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = DetrAttention(config)
        self.self_attn_layer_norm = mindspore.mint.nn.LayerNorm(self.hidden_size)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.fc1 = mindspore.mint.nn.Linear(self.hidden_size, config.encoder_ffn_dim)
        self.fc2 = mindspore.mint.nn.Linear(config.encoder_ffn_dim, self.hidden_size)
        self.final_layer_norm = mindspore.mint.nn.LayerNorm(self.hidden_size)

    def construct(
        self,
        hidden_states: mindspore.Tensor,
        attention_mask: mindspore.Tensor,
        object_queries: mindspore.Tensor,
        output_attentions: Optional[bool] = None,
    ):
        """
        Args:
            hidden_states (`mindspore.Tensor`): input to the layer of shape `(seq_len, batch, embed_dim)`
...
class DabDetrDecoderLayer(nn.Cell):
    def __init__(self, config: DabDetrConfig, is_first: bool = False):
        super().__init__()
...
class DabDetrMLP(mindspore.nn.Cell):
    """
    ...
    def construct(self, input_tensor):
        for i, layer in enumerate(self.layers):
            input_tensor = mindspore.mint.nn.functional.relu(layer(input_tensor)) if i < self.num_layers - 1 else layer(input_tensor)
        return input_tensor
...
class DabDetrEncoder(DabDetrPreTrainedModel):
    ...
    def __init__(self, config: DabDetrConfig):
        ...
        self.layers = mindspore.nn.CellList([DabDetrEncoderLayer(config) for _ in range(config.encoder_layers)])
        self.norm = mindspore.mint.nn.LayerNorm(config.hidden_size) if config.normalize_before else None
    ...
    def construct(
        ...
        r"""
        Args:
            inputs_embeds (`mindspore.Tensor` of shape `(sequence_length, batch_size, hidden_size)`):
            ...
            object_queries (`mindspore.Tensor` of shape `(sequence_length, batch_size, hidden_size)`):
...
class DabDetrDecoder(DabDetrPreTrainedModel):
    ...
    def __init__(self, config: DabDetrConfig):
        ...
        self.hidden_size = config.hidden_size
        self.layernorm = mindspore.mint.nn.LayerNorm(self.hidden_size)
    ...
    def construct(
        ...
        Args:
            ...
            encoder_hidden_states (`mindspore.Tensor` of shape `(encoder_sequence_length, batch_size, hidden_size)`, *optional*):
                Sequence of hidden-states at the output of the last layer of the encoder. Used in the cross-attention
                of the decoder.
            memory_key_padding_mask (`mindspore.Tensor.bool` of shape `(batch_size, sequence_length)`):
            ...
            object_queries (`mindspore.Tensor` of shape `(sequence_length, batch_size, hidden_size)`, *optional*):
                Position embeddings that are added to the queries and keys in each cross-attention layer.
            query_position_embeddings (`mindspore.Tensor` of shape `(num_queries, batch_size, number_of_anchor_points)`):
        ...
        output_intermediate_hidden_states = mindspore.mint.stack(intermediate)
        output_reference_points = mindspore.mint.stack(ref_points)
...
class DabDetrModel(DabDetrPreTrainedModel):
    def __init__(self, config: DabDetrConfig):
        ...
        self.query_refpoint_embeddings = mindspore.mint.nn.Embedding(config.num_queries, config.query_dim)
        ...
        self.input_projection = mindspore.mint.nn.Conv2d(
        ...
        if self.num_patterns > 0:
            self.patterns = mindspore.mint.nn.Embedding(self.num_patterns, self.hidden_size)
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple[mindspore.Tensor], DabDetrModelOutput]:
        r"""
        ...
        if pixel_mask is None:
            pixel_mask = mindspore.mint.ones(((batch_size, height, width)), )
        ...
        if self.num_patterns == 0:
            queries = mindspore.mint.zeros(batch_size, num_queries, self.hidden_size, )
        else:
            queries = (
...
class DabDetrMHAttentionMap(mindspore.nn.Cell):
    ...
    def __init__(self, query_dim, hidden_dim, num_heads, dropout=0.0, bias=True, std=None):
        ...
        self.dropout = mindspore.mint.nn.Dropout(dropout)

        self.q_linear = mindspore.mint.nn.Linear(query_dim, hidden_dim, bias=bias)
        self.k_linear = mindspore.mint.nn.Linear(query_dim, hidden_dim, bias=bias)

        self.normalize_fact = float(hidden_dim / self.num_heads) ** -0.5

    def construct(self, q, k, mask: Optional[Tensor] = None):
        q = self.q_linear(q)
        k = mindspore.mint.nn.functional.conv2d(k, self.k_linear.weight.unsqueeze(-1).unsqueeze(-1), self.k_linear.bias)
...
class DabDetrForObjectDetection(DabDetrPreTrainedModel):
    ...
    def __init__(self, config: DabDetrConfig):
        ...
        self.class_embed = mindspore.mint.nn.Linear(config.hidden_size, config.num_labels)
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[tuple[mindspore.Tensor], DabDetrObjectDetectionOutput]:
        r"""
        decoder_attention_mask (`mindspore.Tensor` of shape `(batch_size, num_queries)`, *optional*):
            Not used by default. Can be used to mask object queries.
        inputs_embeds (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
        ...
        >>> target_sizes = mindspore.tensor([(image.height, image.width)])
