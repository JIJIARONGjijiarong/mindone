...
def binary_mask_to_rle(mask):
    ...
    Args:
        mask (`mindspore.Tensor` or `numpy.array`):
...
def convert_segmentation_to_rle(segmentation):
    ...
    Args:
        segmentation (`mindspore.Tensor` or `numpy.array`):
            A segmentation map of shape `(height, width)` where each value denotes a segment or class id.
    Returns:
        `list[List]`: A list of lists, where each list is the run-length encoding of a segment / class id.
    """
    segment_ids = mindspore.mint.unique(segmentation)

    run_length_encodings = []
    for idx in segment_ids:
        mask = mindspore.mint.where(segmentation == idx, 1, 0)
...
def remove_low_and_no_objects(masks, scores, labels, object_mask_threshold, num_labels):
    ...
    Returns:
        `tuple[`mindspore.Tensor`, `mindspore.Tensor`, `mindspore.Tensor`]`: The `masks`, `scores` and `labels` without the region
...
def compute_segments(
    ...
    segmentation = mindspore.mint.zeros((height, width), dtype=mindspore.int32, )
    segments: list[dict] = []

    if target_size is not None:
        mask_probs = mindspore.mint.nn.functional.interpolate(
...
class DeformableDetrImageProcessor(BaseImageProcessor):
    ...
    def post_process(self, outputs, target_sizes):
        ...
        Args:
            ...
            target_sizes (`mindspore.Tensor` of shape `(batch_size, 2)`):
        ...
        topk_values, topk_indexes = mindspore.mint.topk(prob.view(out_logits.shape[0], -1), 100, dim=1)
        scores = topk_values
        topk_boxes = mindspore.mint.div(topk_indexes, out_logits.shape[2], rounding_mode="floor")
        labels = topk_indexes % out_logits.shape[2]
        boxes = center_to_corners_format(out_bbox)
        boxes = mindspore.mint.gather(boxes, 1, topk_boxes.unsqueeze(-1).repeat(1, 1, 4))

        # and from relative [0, 1] to absolute [0, height] coordinates
        img_h, img_w = target_sizes.unbind(1)
        scale_fct = mindspore.mint.stack([img_w, img_h, img_w, img_h], dim=1)
    ...
    def post_process_object_detection(
        ...
        Args:
            ...
            target_sizes (`mindspore.Tensor` or `list[tuple[int, int]]`, *optional*):
        ...
        topk_values, topk_indexes = mindspore.mint.topk(prob, k_value, dim=1)
        scores = topk_values
        topk_boxes = mindspore.mint.div(topk_indexes, out_logits.shape[2], rounding_mode="floor")
        labels = topk_indexes % out_logits.shape[2]
        boxes = center_to_corners_format(out_bbox)
