...
def convert_coco_poly_to_mask(segmentations, height: int, width: int) -> mindspore.Tensor:
    """
    ...
    for polygons in segmentations:
        ...
        if len(mask.shape) < 3:
            mask = mask[..., None]
        mask = mindspore.tensor(mask, dtype=mindspore.uint8, )
        mask = mindspore.mint.any(mask, axis=2)
        masks.append(mask)
    if masks:
        masks = mindspore.mint.stack(masks, axis=0)
    else:
        masks = mindspore.mint.zeros((0, height, width), dtype=mindspore.uint8, )
...
def prepare_coco_detection_annotation(
    ...
    image_id = mindspore.tensor([image_id], dtype=mindspore.int64, )
    ...
    area = mindspore.tensor(area, dtype=mindspore.float32, )
    iscrowd = mindspore.mint.zeros_like(classes, dtype=mindspore.int64, )
    # guard against no boxes via resizing
    boxes = mindspore.tensor(boxes, dtype=mindspore.float32, ).reshape(-1, 4)
    boxes[:, 2:] += boxes[:, :2]
    ...
    new_target = {
        ...
        "orig_size": mindspore.tensor([int(image_height), int(image_width)], dtype=mindspore.int64, ),
    }

    if keypoints:
        keypoints = mindspore.tensor(keypoints, dtype=mindspore.float32, )
...
def masks_to_boxes(masks: mindspore.Tensor) -> mindspore.Tensor:
    """
    ...
    """
    if masks.numel() == 0:
        return mindspore.mint.zeros((0, 4), )

    h, w = masks.shape[-2:]
    y = mindspore.mint.arange(0, h, dtype=mindspore.float32, )
    x = mindspore.mint.arange(0, w, dtype=mindspore.float32, )
    # see https://github.com/pytorch/pytorch/issues/50276
    y, x = mindspore.mint.meshgrid(y, x, indexing="ij")

    x_mask = masks * mindspore.mint.unsqueeze(x, 0)
    x_max = x_mask.view(x_mask.shape[0], -1).max(-1)[0]
    x_min = (
        mindspore.mint.where(masks, x.unsqueeze(0), mindspore.Tensor(1e8, )).view(masks.shape[0], -1).min(-1)[0]
    )

    y_mask = masks * mindspore.mint.unsqueeze(y, 0)
    y_max = y_mask.view(y_mask.shape[0], -1).max(-1)[0]
    y_min = (
        mindspore.mint.where(masks, y.unsqueeze(0), mindspore.Tensor(1e8, )).view(masks.shape[0], -1).min(-1)[0]
    )

    return mindspore.mint.stack([x_min, y_min, x_max, y_max], 1)
...
def rgb_to_id(color):
    ...
    """
    if isinstance(color, mindspore.Tensor) and len(color.shape) == 3:
        if color.dtype == mindspore.uint8:
            color = color.to(mindspore.int32)
        return color[:, :, 0] + 256 * color[:, :, 1] + 256 * 256 * color[:, :, 2]
    return int(color[0] + 256 * color[1] + 256 * 256 * color[2])


def prepare_coco_panoptic_annotation(
    image: mindspore.Tensor,
    target: dict,
    ...
    new_target = {}
    new_target["image_id"] = mindspore.tensor(
        [target["image_id"] if "image_id" in target else target["id"]], dtype=mindspore.int64, )
    new_target["size"] = mindspore.tensor([image_height, image_width], dtype=mindspore.int64, )
...
class DeformableDetrImageProcessorFast(BaseImageProcessorFast):
    ...
    def resize_annotation(
        ...
        for key, value in annotation.items():
            if key == "boxes":
                boxes = value
                scaled_boxes = boxes * mindspore.tensor(
                    [ratio_width, ratio_height, ratio_width, ratio_height], dtype=mindspore.float32, )
                ...
                masks = mindspore.mint.stack(masks).to(mindspore.float32)
        ...
        for key, value in annotation.items():
            if key == "boxes":
                ...
                boxes /= mindspore.tensor(
                    [image_width, image_height, image_width, image_height], dtype=mindspore.float32, )
    ...
    def _update_annotation_for_padded_image(
        ...
        for key, value in annotation.items():
            if key == "masks":
                ...
                boxes = value
                boxes *= mindspore.tensor([ratio_width, ratio_height, ratio_width, ratio_height], )
    ...
    def pad(
        ...
        pixel_mask = mindspore.mint.zeros(padded_size, dtype=mindspore.int64, )
        ...
        if do_pad:
            ...
            for image, annotation in zip(images, annotations if annotations is not None else [None] * len(images)):
                ...
                if padded_size == image.shape[-2:]:
                    ...
                    pixel_masks.append(mindspore.mint.ones(padded_size, dtype=mindspore.int64, ))
            ...
            data.update({"pixel_mask": mindspore.mint.stack(pixel_masks, dim=0)})

        data.update({"pixel_values": mindspore.mint.stack(images, dim=0)})
