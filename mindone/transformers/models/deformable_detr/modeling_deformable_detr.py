...
class MultiScaleDeformableAttention(mindspore.nn.Cell):
    def construct(
        ...
        for level_id, (height, width) in enumerate(value_spatial_shapes_list):
            ...
            sampling_value_l_ = mindspore.mint.nn.functional.grid_sample(
                value_l_,
                sampling_grid_l_,
                mode="bilinear",
                padding_mode="zeros",
        ...
        output = (
            (mindspore.mint.stack(sampling_value_list, dim=-2).flatten(-2) * attention_weights)
            .sum(-1)
...
class DeformableDetrDecoderOutput(nn.Cell):
    r"""
...
class DeformableDetrModelOutput(nn.Cell):
    r"""
...
def _get_clones(module, N):
    return mindspore.nn.CellList([copy.deepcopy(module) for i in range(N)])
...
class DeformableDetrFrozenBatchNorm2d(mindspore.nn.Cell):
    """
...
class DeformableDetrConvEncoder(mindspore.nn.Cell):
    """
...
class DeformableDetrConvModel(mindspore.nn.Cell):
    """
...
class DeformableDetrSinePositionEmbedding(mindspore.nn.Cell):
    """
    ...
    def construct(self, pixel_values, pixel_mask):
        ...
        dim_t = mindspore.mint.arange(self.embedding_dim, dtype=pixel_values.dtype, )
        dim_t = self.temperature ** (2 * mindspore.mint.div(dim_t, 2, rounding_mode="floor") / self.embedding_dim)
...
class DeformableDetrLearnedPositionEmbedding(mindspore.nn.Cell):
    """
    ...
    def __init__(self, embedding_dim=256):
        super().__init__()
        self.row_embeddings = mindspore.mint.nn.Embedding(50, embedding_dim)
        self.column_embeddings = mindspore.mint.nn.Embedding(50, embedding_dim)

    def construct(self, pixel_values, pixel_mask=None):
        height, width = pixel_values.shape[-2:]
        width_values = mindspore.mint.arange(width, )
        height_values = mindspore.mint.arange(height, )
        x_emb = self.column_embeddings(width_values)
        y_emb = self.row_embeddings(height_values)
        pos = mindspore.mint.cat([x_emb.unsqueeze(0).repeat(height, 1, 1), y_emb.unsqueeze(1).repeat(1, width, 1)], dim=-1)
...
class DeformableDetrMultiscaleDeformableAttention(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: DeformableDetrConfig, num_heads: int, n_points: int):
        ...
        self.sampling_offsets = mindspore.mint.nn.Linear(config.d_model, num_heads * self.n_levels * n_points * 2)
        self.attention_weights = mindspore.mint.nn.Linear(config.d_model, num_heads * self.n_levels * n_points)
        self.value_proj = mindspore.mint.nn.Linear(config.d_model, config.d_model)
        self.output_proj = mindspore.mint.nn.Linear(config.d_model, config.d_model)
    ...
    def construct(
        ...
        attention_weights = mindspore.mint.nn.functional.softmax(attention_weights, -1).view(
            batch_size, num_queries, self.n_heads, self.n_levels, self.n_points
        )
        # batch_size, num_queries, n_heads, n_levels, n_points, 2
        num_coordinates = reference_points.shape[-1]
        if num_coordinates == 2:
            offset_normalizer = mindspore.mint.stack([spatial_shapes[..., 1], spatial_shapes[..., 0]], -1)
            sampling_locations = (
...
class DeformableDetrMultiheadAttention(mindspore.nn.Cell):
    """
...
class DeformableDetrEncoderLayer(nn.Cell):
...
class DeformableDetrDecoderLayer(nn.Cell):
    def __init__(self, config: DeformableDetrConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.encoder_attn_layer_norm = mindspore.mint.nn.LayerNorm(self.embed_dim)
        # feedforward neural networks
        self.fc1 = mindspore.mint.nn.Linear(self.embed_dim, config.decoder_ffn_dim)
        self.fc2 = mindspore.mint.nn.Linear(config.decoder_ffn_dim, self.embed_dim)
        self.final_layer_norm = mindspore.mint.nn.LayerNorm(self.embed_dim)
...
class DeformableDetrPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, DeformableDetrLearnedPositionEmbedding):
            ...
            grid_init = mindspore.mint.stack([thetas.cos(), thetas.sin()], -1)
            grid_init = (
...
class DeformableDetrEncoder(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        self.layers = mindspore.nn.CellList([DeformableDetrEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def get_reference_points(spatial_shapes, valid_ratios):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ref_y, ref_x = meshgrid(
                mindspore.mint.linspace(0.5, height - 0.5, height, dtype=valid_ratios.dtype, ),
                mindspore.mint.linspace(0.5, width - 0.5, width, dtype=valid_ratios.dtype, ),
                indexing="ij",
            )
            # TODO: valid_ratios could be useless here. check https://github.com/fundamentalvision/Deformable-DETR/issues/36
            ref_y = ref_y.reshape(-1)[None] / (valid_ratios[:, None, level, 1] * height)
            ref_x = ref_x.reshape(-1)[None] / (valid_ratios[:, None, level, 0] * width)
            ref = mindspore.mint.stack((ref_x, ref_y), -1)
            reference_points_list.append(ref)
        reference_points = mindspore.mint.cat(reference_points_list, 1)
    ...
    def construct(
        ...
        reference_points = self.get_reference_points(spatial_shapes_tuple, valid_ratios, )
...
class DeformableDetrDecoder(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        self.layers = mindspore.nn.CellList([DeformableDetrDecoderLayer(config) for _ in range(config.decoder_layers)])
        self.gradient_checkpointing = False
    ...
    def construct(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if num_coordinates == 4:
                reference_points_input = (
                    reference_points[:, :, None] * mindspore.mint.cat([valid_ratios, valid_ratios], -1)[:, None]
                )
        ...
        intermediate = mindspore.mint.stack(intermediate, dim=1)
        intermediate_reference_points = mindspore.mint.stack(intermediate_reference_points, dim=1)
...
class DeformableDetrModel(DeformableDetrPreTrainedModel):
    def __init__(self, config: DeformableDetrConfig):
        ...
        if config.num_feature_levels > 1:
            ...
            for _ in range(num_backbone_outs):
                ...
                input_proj_list.append(
                    mindspore.nn.SequentialCell(
                        mindspore.mint.nn.Conv2d(in_channels, config.d_model, kernel_size=1),
                        mindspore.mint.nn.GroupNorm(32, config.d_model),
                    )
                )
            ...
            self.input_proj = mindspore.nn.CellList(input_proj_list)
        else:
            self.input_proj = mindspore.nn.CellList(
                [
                    mindspore.nn.SequentialCell(
                        mindspore.mint.nn.Conv2d(
        ...
        if not config.two_stage:
            self.query_position_embeddings = mindspore.mint.nn.Embedding(config.num_queries, config.d_model * 2)
        ...
        if config.two_stage:
            ...
            self.enc_output_norm = mindspore.mint.nn.LayerNorm(config.d_model)
            self.pos_trans = mindspore.mint.nn.Linear(config.d_model * 2, config.d_model * 2)
            self.pos_trans_norm = mindspore.mint.nn.LayerNorm(config.d_model * 2)
        else:
            self.reference_points = mindspore.mint.nn.Linear(config.d_model, 2)
    ...
    def get_valid_ratio(self, mask, dtype=mindspore.float32):
        ...
        valid_height = mindspore.mint.sum(mask[:, :, 0], 1)
        valid_width = mindspore.mint.sum(mask[:, 0, :], 1)
        valid_ratio_height = valid_height.to(dtype) / height
        valid_ratio_width = valid_width.to(dtype) / width
        valid_ratio = mindspore.mint.stack([valid_ratio_width, valid_ratio_height], -1)
        return valid_ratio
    ...
    def get_proposal_pos_embed(self, proposals):
        ...
        dim_t = mindspore.mint.arange(num_pos_feats, dtype=proposals.dtype, )
        dim_t = temperature ** (2 * mindspore.mint.div(dim_t, 2, rounding_mode="floor") / num_pos_feats)
        # batch_size, num_queries, 4
        proposals = proposals.sigmoid() * scale
        # batch_size, num_queries, 4, 128
        pos = proposals[:, :, :, None] / dim_t
        # batch_size, num_queries, 4, 64, 2 -> batch_size, num_queries, 512
        pos = mindspore.mint.stack((pos[:, :, :, 0::2].sin(), pos[:, :, :, 1::2].cos()), dim=4).flatten(2)
        return pos
    ...
    def gen_encoder_output_proposals(self, enc_output, padding_mask, spatial_shapes):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            valid_height = mindspore.mint.sum(~mask_flatten_[:, :, 0, 0], 1)
            valid_width = mindspore.mint.sum(~mask_flatten_[:, 0, :, 0], 1)
            ...
            grid = mindspore.mint.cat([grid_x.unsqueeze(-1), grid_y.unsqueeze(-1)], -1)

            scale = mindspore.mint.cat([valid_width.unsqueeze(-1), valid_height.unsqueeze(-1)], 1).view(batch_size, 1, 1, 2)
            grid = (grid.unsqueeze(0).expand(batch_size, -1, -1, -1) + 0.5) / scale
            width_height = mindspore.mint.ones_like(grid) * 0.05 * (2.0**level)
            proposal = mindspore.mint.cat((grid, width_height), -1).view(batch_size, -1, 4)
            proposals.append(proposal)
            _cur += height * width
        output_proposals = mindspore.mint.cat(proposals, 1)
        output_proposals_valid = ((output_proposals > 0.01) & (output_proposals < 0.99)).all(-1, keepdim=True)
        output_proposals = mindspore.mint.log(output_proposals / (1 - output_proposals))  # inverse sigmoid
    ...
    def construct(
        ...
        if pixel_mask is None:
            pixel_mask = mindspore.mint.ones(((batch_size, height, width)), dtype=mindspore.int64, )
        ...
        if self.config.num_feature_levels > len(sources):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                mask = mindspore.mint.nn.functional.interpolate(pixel_mask[None].to(pixel_values.dtype), size=source.shape[-2:]).to(
                    mindspore.bool_
                )[0]
        ...
        source_flatten = mindspore.mint.cat(source_flatten, 1)
        mask_flatten = mindspore.mint.cat(mask_flatten, 1)
        lvl_pos_embed_flatten = mindspore.mint.cat(lvl_pos_embed_flatten, 1)
        spatial_shapes = mindspore.tensor(spatial_shapes_list, dtype=mindspore.int64, )
        level_start_index = mindspore.mint.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        valid_ratios = mindspore.mint.stack([self.get_valid_ratio(m, dtype=source_flatten.dtype) for m in masks], 1)
        ...
        if self.config.two_stage:
            ...
            topk_proposals = mindspore.mint.topk(enc_outputs_class[..., 0], topk, dim=1)[1]
            topk_coords_logits = mindspore.mint.gather(
            ...
            query_embed, target = mindspore.mint.split(pos_trans_out, num_channels, dim=2)
        else:
            query_embed, target = mindspore.mint.split(query_embeds, num_channels, dim=1)
...
class DeformableDetrMLPPredictionHead(mindspore.nn.Cell):
    """
    ...
    def construct(self, x):
        for i, layer in enumerate(self.layers):
            x = mindspore.mint.nn.functional.relu(layer(x)) if i < self.num_layers - 1 else layer(x)
        return x
...
class DeformableDetrForObjectDetection(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.class_embed = mindspore.mint.nn.Linear(config.d_model, config.num_labels)
    ...
    def construct(
        ...
        outputs_class = mindspore.mint.stack(outputs_classes)
        outputs_coord = mindspore.mint.stack(outputs_coords)
