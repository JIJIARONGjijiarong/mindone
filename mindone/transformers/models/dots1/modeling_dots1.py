...
from transformers.models.dots1 import Dots1Config
...
class Dots1RMSNorm(mindspore.nn.Cell):
...
class Dots1RotaryEmbedding(mindspore.nn.Cell):
...
class Dots1Attention(mindspore.nn.Cell):
...
class Dots1MLP(mindspore.nn.Cell):
    def __init__(self, config, hidden_size=None, intermediate_size=None):
        ...
        self.gate_proj = mindspore.mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.up_proj = mindspore.mint.nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.down_proj = mindspore.mint.nn.Linear(self.intermediate_size, self.hidden_size, bias=False)
...
class Dots1MoE(mindspore.nn.Cell):
    """
    ...
    def moe(self, hidden_states: mindspore.Tensor, topk_indices: mindspore.Tensor, topk_weights: mindspore.Tensor):
        ...
        for expert_idx in range(len(self.experts)):
            ...
            if token_indices.numel() > 0:
                ...
                # Use mindspore ops instead of index_add_
                final_hidden_states = final_hidden_states.scatter_add(
                    0,
                    token_indices.unsqueeze(-1).broadcast_to(token_indices.shape[0], weighted_output.shape[-1]),
                    weighted_output,
                )
...
class Dots1TopkRouter(mindspore.nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.weight = mindspore.Parameter(mindspore.mint.empty((self.n_routed_experts, config.hidden_size)))
        self.e_score_correction_bias = mindspore.Parameter(
            mindspore.mint.zeros(self.n_routed_experts),
            requires_grad=False,
        )
    ...
    def get_topk_indices(self, scores):
        ...
        group_idx = mindspore.mint.topk(group_scores, k=self.topk_group, dim=-1, sorted=False)[1]
        group_mask = mindspore.mint.zeros_like(group_scores)
        group_mask = group_mask.scatter(1, group_idx, mindspore.mint.ones_like(group_idx, dtype=group_mask.dtype))
        score_mask = (
        ...
        topk_indices = mindspore.mint.topk(scores_for_choice, k=self.top_k, dim=-1, sorted=False)[1]
        return topk_indices

    def construct(self, hidden_states):
        hidden_states = hidden_states.view(-1, self.config.hidden_size)
        router_logits = mindspore.mint.nn.Linear(hidden_states.to(mindspore.float32), self.weight.to(mindspore.float32))
...
class Dots1DecoderLayer(mindspore.nn.Cell):  # TODO: GradientCheckpointingLayer
    def __init__(self, config: Dots1Config, layer_idx: int):
        super().__init__()
