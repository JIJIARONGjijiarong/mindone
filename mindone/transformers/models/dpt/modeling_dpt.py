...
class DPTViTHybridEmbeddings(nn.Cell):
    ...
    def __init__(self, config, feature_size=None):
        ...
        self.projection = mint.nn.Conv2d(feature_dim, hidden_size, kernel_size=1, has_bias=True, pad_mode="valid")
    ...
    def _resize_pos_embed(self, posemb, posemb_shape, grid_size_height, grid_size_width, start_index=1):
        ...
        posemb_grid = mint.nn.functional.interpolate(posemb_grid, size=(grid_size_height, grid_size_width), mode="bilinear")
    ...
    def construct(self, hidden_state, residual=None):
        if residual is not None:
            if hidden_state.shape != residual.shape:
                residual = mint.nn.functional.interpolate(
...
class DPTForDepthEstimation(DPTPreTrainedModel):
    ...
    def construct(
        ...
        >>> prediction = ms.mint.nn.functional.interpolate(
