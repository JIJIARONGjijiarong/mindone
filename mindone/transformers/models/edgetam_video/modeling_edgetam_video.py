...
class EdgeTamVideoLayerNorm(mint.nn.LayerNorm):
...
class EdgeTamVideoMemoryFuserCXBlock(GradientCheckpointingLayer):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        self.depthwise_conv = mint.nn.Conv2d(
        ...
        self.pointwise_conv1 = mint.nn.Linear(
...
class EdgeTamVideoVisionEncoderOutput(ModelOutput):
    ...
    fpn_hidden_states: Optional[ms.Tensor] = None
    fpn_position_encoding: Optional[ms.Tensor] = None
    hidden_states: Optional[tuple[ms.Tensor, ...]] = None
    attentions: Optional[tuple[ms.Tensor, ...]] = None


class EdgeTamVideoVisionRotaryEmbedding(ms.nn.Cell):
    """
    ...
    @ms._no_grad()
    def construct(self) -> tuple[ms.Tensor, ms.Tensor]:
...
class EdgeTamVideoAttention(ms.nn.Cell):
    """
...
class EdgeTamVideoTwoWayAttentionBlock(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoMaskDecoderConfig, skip_first_layer_pe: bool = False):
        ...
        self.layer_norm1 = mint.nn.LayerNorm(config.hidden_size)

        self.cross_attn_token_to_image = EdgeTamVideoAttention(config)
        self.layer_norm2 = mint.nn.LayerNorm(config.hidden_size)

        self.mlp = EdgeTamVideoFeedForward(
            config.hidden_size, config.mlp_dim, config.hidden_size, num_layers=config.num_hidden_layers
        )
        self.layer_norm3 = mint.nn.LayerNorm(config.hidden_size)

        self.layer_norm4 = mint.nn.LayerNorm(config.hidden_size)
...
class EdgeTamVideoPositionEmbeddingSine(ms.nn.Cell):
    """
    ...
    def construct(
        ...
        if mask is None:
            mask = mint.zeros((shape[0], shape[2], shape[3]), dtype=ms.bool_)
        ...
        dim_t = mint.arange(self.num_pos_feats, dtype=ms.int64, ).to(dtype)
...
class EdgeTamVideoMemoryFuser(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
...
class EdgeTamVideoMaskDownSamplerLayer(ms.nn.Cell):
...
class EdgeTamVideoMaskDownSampler(ms.nn.Cell):
    """
    ...
    def __init__(self, config: EdgeTamVideoConfig):
        ...
        self.final_conv = mint.nn.Conv2d(mask_out_chans, config.mask_downsampler_embed_dim, kernel_size=1)
...
class EdgeTamVideoMemoryEncoder(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        ...
        self.feature_projection = mint.nn.Conv2d(hidden_size, hidden_size, kernel_size=1)
...
class EdgeTamVideoFeedForward(ms.nn.Cell):
    def __init__(
        self,
        input_dim: int,
        hidden_dim: int,
        ...
        self.proj_in = mint.nn.Linear(input_dim, hidden_dim)
        self.proj_out = mint.nn.Linear(hidden_dim, output_dim)
        self.layers = ms.nn.CellList([mint.nn.Linear(hidden_dim, hidden_dim) for _ in range(num_layers - 2)])
    ...
    def construct(self, hidden_states):
        ...
        if self.sigmoid_output:
            hidden_states = mint.nn.functional.sigmoid(hidden_states)
        return hidden_states
...
class EdgeTamVideoInferenceCache:
    ...
    def cache_vision_features(self, frame_idx: int, features: dict):
        ...
        for key, value in features.items():
            ...
            elif isinstance(value, (list, tuple)) and value and isinstance(value[0], ms.Tensor):
...
class EdgeTamVideoInferenceSession:
    ...
    def add_mask_inputs(self, obj_idx: int, frame_idx: int, inputs: ms.Tensor):
    ...
    def store_output(
        ...
        output_value: Optional[Union[ms.Tensor, dict]] = None,
    ...
    def add_new_frame(self, pixel_values: ms.Tensor, frame_idx: Optional[int] = None) -> int:
    ...
    def get_frame(self, frame_idx: int) -> ms.Tensor:
...
class EdgeTamVideoMemoryAttention(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        self.layers = ms.nn.CellList(
            [EdgeTamVideoMemoryAttentionLayer(config) for _ in range(config.memory_attention_num_layers)]
        )
        self.layer_norm = mint.nn.LayerNorm(config.memory_attention_hidden_size)
...
def window_partition(hidden_state, window_size):
    ...
    hidden_state = mint.nn.functional.pad(hidden_state, (0, 0, 0, pad_width, 0, pad_height))
...
class EdgeTamVideoImageSegmentationOutput(ModelOutput):
    ...
    iou_scores: Optional[ms.Tensor] = None
    pred_masks: Optional[ms.Tensor] = None
    object_score_logits: Optional[ms.Tensor] = None
    image_embeddings: tuple[ms.Tensor, ...] = None
    vision_hidden_states: Optional[tuple[ms.Tensor, ...]] = None
    vision_attentions: Optional[tuple[ms.Tensor, ...]] = None
    mask_decoder_attentions: Optional[tuple[ms.Tensor, ...]] = None

    high_res_masks: Optional[ms.Tensor] = None
    object_pointer: Optional[ms.Tensor] = None
...
class EdgeTamVideoPositionalEmbedding(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoPromptEncoderConfig):
        super().__init__()
        self.scale = config.scale
        positional_embedding = self.scale * mint.randn((2, config.hidden_size // 2))
    ...
    def construct(self, input_coords, input_shape=None):
        ...
        return mint.cat([mint.sin(coordinates), mint.cos(coordinates)], dim=-1)


class EdgeTamVideoMaskEmbedding(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoPromptEncoderConfig):
        super().__init__()
        self.mask_input_channels = config.mask_input_channels // 4
        self.activation = ACT2FN[config.hidden_act]
        self.conv1 = mint.nn.Conv2d(1, self.mask_input_channels, kernel_size=2, stride=2)
        self.conv2 = mint.nn.Conv2d(self.mask_input_channels, config.mask_input_channels, kernel_size=2, stride=2)
        self.conv3 = mint.nn.Conv2d(config.mask_input_channels, config.hidden_size, kernel_size=1)
...
class EdgeTamVideoPromptEncoder(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoPromptEncoderConfig):
        ...
        self.point_embed = mint.nn.Embedding(config.num_point_embeddings, config.hidden_size)
        self.hidden_size = config.hidden_size
        self.not_a_point_embed = mint.nn.Embedding(1, config.hidden_size)
    ...
    def _embed_points(self, points: ms.Tensor, labels: ms.Tensor, pad: bool) -> ms.Tensor:
        ...
        point_embedding = mint.where(labels[..., None] == -1, self.not_a_point_embed.weight, point_embedding)
        ...
        point_embedding = mint.where(
            labels[..., None] != -10,
            point_embedding,
            mint.zeros_like(point_embedding),
        )
    ...
    def construct(
        ...
        if input_boxes is not None:
            ...
            if sparse_embeddings is None:
                ...
                sparse_embeddings = mint.cat([sparse_embeddings, box_embeddings], dim=2)
...
class EdgeTamVideoTwoWayTransformer(ms.nn.Cell):
...
class EdgeTamVideoMaskDecoder(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoMaskDecoderConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        self.iou_token = mint.nn.Embedding(1, self.hidden_size)
        self.mask_tokens = mint.nn.Embedding(self.num_mask_tokens, self.hidden_size)
        ...
        self.output_hypernetworks_mlps = ms.nn.CellList(mlps_list)
        ...
        self.conv_s0 = mint.nn.Conv2d(config.hidden_size, config.hidden_size // 8, kernel_size=1, stride=1)
        self.conv_s1 = mint.nn.Conv2d(config.hidden_size, config.hidden_size // 4, kernel_size=1, stride=1)

        self.obj_score_token = mint.nn.Embedding(1, self.hidden_size)
    ...
    def construct(
        ...
        high_resolution_features: list[ms.Tensor],
        attention_similarity: Optional[ms.Tensor] = None,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if sparse_prompt_embeddings.shape[0] != 0:
            tokens = mint.cat((output_tokens, sparse_prompt_embeddings), dim=2)
        else:
        ...
        hyper_in_list: list[ms.Tensor] = []
        for i in range(self.num_mask_tokens):
            current_mlp = self.output_hypernetworks_mlps[i]
            hyper_in_list += [current_mlp(mask_tokens_out[:, :, i, :])]
        hyper_in = mint.stack(hyper_in_list, dim=2)
    ...
    def _get_stability_scores(self, mask_logits):
        ...
        area_i = mint.sum(mask_logits > stability_delta, dim=-1).float()
        area_u = mint.sum(mask_logits > -stability_delta, dim=-1).float()
        stability_scores = mint.where(area_u > 0, area_i / area_u, 1.0)
    ...
    def _dynamic_multimask_via_stability(self, all_mask_logits, all_iou_scores):
        ...
        mask_logits_out = mint.where(
            is_stable[..., None, None].expand_as(singlemask_logits),
            singlemask_logits,
            best_multimask_logits,
        )
        iou_scores_out = mint.where(
...
class EdgeTamVideoModel(EdgeTamVideoPreTrainedModel):
    ...
    def __init__(self, config: EdgeTamVideoConfig):
        ...
        self.mask_downsample = mint.nn.Conv2d(1, 1, kernel_size=4, stride=4)
        ...
        if self.config.enable_temporal_pos_encoding_for_object_pointers:
            ...
            self.temporal_positional_encoding_projection_layer = mint.nn.Linear(self.hidden_dim, self.mem_dim)
        else:
            self.temporal_positional_encoding_projection_layer = mint.nn.Identity()
    ...
    def get_image_wide_positional_embeddings(self) -> ms.Tensor:
        ...
        positional_embedding = self.shared_image_embedding(mint.stack([x_embed, y_embed], dim=-1))
    ...
    def get_prompt_embeddings(
        self,
        input_points: Optional[ms.Tensor] = None,
        input_labels: Optional[ms.Tensor] = None,
        input_boxes: Optional[ms.Tensor] = None,
        input_masks: Optional[ms.Tensor] = None,
    ) -> tuple[ms.Tensor, ms.Tensor]:
        r"""
    ...
    def construct(
        ...
        if len(pred_masks_per_obj) > 1:
            all_pred_masks = mint.cat(pred_masks_per_obj, dim=0)
        else:
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if input_masks is not None:
            ...
            if input_masks.shape[-2:] != self.prompt_encoder.mask_input_size:
                input_masks = mint.nn.functional.interpolate(
                    input_masks.float(),
    ...
    def _use_mask_as_output(
        self,
        backbone_features: ms.Tensor,
        high_res_features: list[ms.Tensor],
        mask_inputs: ms.Tensor,
    ) -> EdgeTamVideoImageSegmentationOutput:
        """
        ...
        low_res_masks = mint.nn.functional.interpolate(
        ...
        is_obj_appearing = mint.any(mask_inputs.flatten(1).float() > 0.0, dim=1)
    ...
    def _process_object_pointers(
        ...
        object_pointers = mint.stack(pointer_tokens, dim=0)
        ...
        if self.config.enable_temporal_pos_encoding_for_object_pointers:
            ...
            normalized_temporal_diffs = (
                ms.Tensor(temporal_offsets, dtype=ms.float32) / max_temporal_diff
            )
    ...
    def _run_single_frame_inference(
        ...
        point_inputs: Optional[ms.Tensor],
        mask_inputs: Optional[ms.Tensor],
        reverse: bool,
        run_mem_encoder: bool,
        prev_sam_mask_logits: Optional[ms.Tensor] = None,
        streaming: bool = False,
    ) -> dict[str, Any]:
        """
