...
class EdgeTamVideoVisionRotaryEmbedding(Sam2VideoVisionRotaryEmbedding):
    def __init__(self, config: EdgeTamVideoConfig, end_x: Optional[int] = None, end_y: Optional[int] = None):
        ...
        freqs = 1.0 / (config.memory_attention_rope_theta ** (mint.arange(0, dim, 4)[: (dim // 4)].float() / dim))

        # Generate 2D position indices for axial rotary embedding
        flattened_indices = mint.arange(end_x * end_y, dtype=ms.int64)
        x_positions = flattened_indices % end_x
        y_positions = mint.div(flattened_indices, end_x, rounding_mode="floor")
        freqs_x = mint.outer(x_positions, freqs).float()
        freqs_y = mint.outer(y_positions, freqs).float()
        inv_freq = mint.cat([freqs_x, freqs_y], dim=-1)
...
def apply_rotary_pos_emb_2d_cross_attn(
    ...
    k_final = mint.cat([k_temporal_reshaped, k_spatial_reshaped], dim=3)
    k_final = k_final.view(batch_size, num_heads, k_seq_len, channels_per_head)

    # Combine RoPE-processed keys with excluded tokens
    k_embed = mint.cat([k_final.type_as(k), k_excluded], dim=-2)
    return q_embed.type_as(q), k_embed


class EdgeTamVideoRoPESelfAttention(ms.nn.Cell):
    ...
    def __init__(self, config: EdgeTamVideoConfig):
        ...
        self.q_proj = mint.nn.Linear(self.hidden_size, self.internal_dim)
        ...
        self.o_proj = mint.nn.Linear(self.internal_dim, self.hidden_size)
...
class EdgeTamVideoRoPECrossAttention(ms.nn.Cell):
    ...
    def construct(
        ...
        value: ms.Tensor,
        position_embeddings: tuple[ms.Tensor, ms.Tensor],
        position_embeddings_k: tuple[ms.Tensor, ms.Tensor],
        num_k_exclude_rope: int = 0,
        rope_k_repeat: int = 0,
        **kwargs: Unpack[FlashAttentionKwargs],
    ) -> Tensor:
...
class EdgeTamVideoPositionEmbeddingSine(Sam2VideoPositionEmbeddingSine):
    ...
    def construct(self, **super_kwargs):
        return super().construct(**super_kwargs)
...
class EdgeTamVideoMemoryAttentionMLP(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.memory_attention_hidden_size
        self.intermediate_size = config.memory_attention_mlp_hidden_size
        self.up_proj = mint.nn.Linear(self.hidden_size, self.intermediate_size)
        self.down_proj = mint.nn.Linear(self.intermediate_size, self.hidden_size)
        self.dropout = mint.nn.Dropout(config.memory_attention_dropout)
...
class EdgeTamVideoMemoryAttentionLayer(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        ...
        self.layer_norm1 = mint.nn.LayerNorm(hidden_size)
        self.layer_norm2 = mint.nn.LayerNorm(hidden_size)
        self.layer_norm3 = mint.nn.LayerNorm(hidden_size)
        self.dropout1 = mint.nn.Dropout(config.memory_attention_dropout)
        self.dropout2 = mint.nn.Dropout(config.memory_attention_dropout)
        self.dropout3 = mint.nn.Dropout(config.memory_attention_dropout)
...
class EdgeTamVideoPerceiverMLP(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        ...
        self.layer_norm = mint.nn.LayerNorm(self.hidden_size)
...
class EdgeTamVideoPerceiverAttention(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.perceiver_resampler_hidden_size
        ...
        self.q_proj = mint.nn.Linear(self.hidden_size, self.inner_dim, bias=False)
        self.k_proj = mint.nn.Linear(self.hidden_size, self.inner_dim, bias=False)
        self.v_proj = mint.nn.Linear(self.hidden_size, self.inner_dim, bias=False)
        self.o_proj = mint.nn.Linear(self.inner_dim, self.hidden_size, bias=False)
    ...
    def construct(
        ...
        key: ms.Tensor,
        value: ms.Tensor,
        positional_encoding: Optional[ms.Tensor] = None,
        **kwargs,
    ) -> ms.Tensor:
...
class EdgeTamVideoPerceiverEncoderLayer(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()

        self.cross_attention = EdgeTamVideoPerceiverAttention(config)
        self.mlp = EdgeTamVideoPerceiverMLP(config)
        self.dropout = mint.nn.Dropout(config.perceiver_resampler_hidden_dropout)
        ...
        self.layer_norm_input = mint.nn.LayerNorm(config.perceiver_resampler_hidden_size)
        self.layer_norm_latents = mint.nn.LayerNorm(config.perceiver_resampler_hidden_size)
        self.layer_norm_self = mint.nn.LayerNorm(config.perceiver_resampler_hidden_size)
...
class EdgeTamVideoPerceiverResampler(ms.nn.Cell):
    def __init__(self, config: EdgeTamVideoConfig):
        super().__init__()
        self.config = config
        self.hidden_size = config.perceiver_resampler_hidden_size
        ...
        if self.num_latents_1d > 0:
            self.latents_1d = ms.Parameter(mint.randn(self.num_latents_1d, self.hidden_size))
        if self.num_latents_2d > 0:
            self.latents_2d = ms.Parameter(mint.randn(self.num_latents_2d, self.hidden_size))

        self.positional_encoding = EdgeTamVideoPositionEmbeddingSine(
            num_pos_feats=self.hidden_size // 2, normalize=True
        )

        self.layers = ms.nn.CellList([EdgeTamVideoPerceiverEncoderLayer(config) for _ in range(self.num_layers)])
    ...
    def construct(
        ...
        combined_latents = mint.cat(output_latents, dim=1)

        combined_positional_encoding = None
        if positional_encoding is not None and output_positional_encodings:
            combined_positional_encoding = mint.cat(output_positional_encodings, dim=1)
    ...
    def _forward_1d(
        ...
        if positional_encoding is not None:
            output_positional_encoding = mint.zeros_like(latents)
    ...
    def _forward_2d(self, hidden_states: ms.Tensor) -> tuple[ms.Tensor, ms.Tensor]:
        ...
        positional_encoding_2d = self.positional_encoding(latents_2d.shape, latents_2d.dtype).to(
...
class EdgeTamVideoModel(Sam2VideoModel):
    ...
    def _prepare_memory_conditioned_features(
        ...
        current_vision_features: list[ms.Tensor],
        current_vision_positional_embeddings: list[ms.Tensor],
        ...
        combined_memory = mint.cat(memories_to_concatenate, dim=0)
        combined_memory_positional_embeddings = mint.cat(memory_positional_embeddings_to_concatenate, dim=0)
    ...
    def _encode_new_memory(
        self,
        current_vision_feats: ms.Tensor,
        pred_masks_high_res: ms.Tensor,
        object_score_logits: ms.Tensor,
        ...
        if is_mask_from_pts and not self.training:
            ...
            mask_for_mem = mint.sigmoid(pred_masks_high_res)
