...
class FalconHybridMambaAttentionDynamicCache(Cache):
    ...
    def __init__(
        ...
        batch_size: int,
        dtype: mindspore.dtype = mindspore.float16,
    ...
    def update_conv_state(
        self,
        layer_idx: int,
        new_conv_state: mindspore.Tensor,
...
class FalconH1Attention(mindspore.nn.Cell):
...
class FalconH1RMSNormGated(mindspore.nn.Cell):
    ...
    def construct(self, hidden_states, gate=None):
        ...
        if not self.norm_before_gate and gate is not None:
            hidden_states = hidden_states * mint.nn.functional.silu(gate.to(mindspore.float32))
...
class FalconH1Mixer(mindspore.nn.Cell):
    """
    ...
    def __init__(self, config: FalconH1Config, layer_idx: int):
        ...
        self.dt_bias = mindspore.Parameter(mint.ones(self.num_heads))

        # S4D real initialization. These are not discretized!
        # The core is to load them, compute the discrete states, then write the updated state. Keeps the memory bounded
        A = mint.arange(1, self.num_heads + 1)
        self.A_log = mindspore.Parameter(mint.log(A))
        ...
        self.D = mindspore.Parameter(mint.ones(self.num_heads))
        self.D._no_weight_decay = True

        self.out_proj = mint.nn.Linear(self.intermediate_size, config.hidden_size, bias=config.projectors_bias)
    ...
    def forward(
        ...
        if self.mamba_rms_norm:
            ...
            scan_output = y * mint.nn.functional.silu(gate)
...
class FalconH1RMSNorm(mindspore.nn.Cell):
...
class FalconH1DecoderLayer(GradientCheckpointingLayer):
    ...
    def construct(
        ...
        hidden_states: mindspore.Tensor,
        attention_mask: Optional[mindspore.Tensor] = None,
        mamba_attention_mask: Optional[mindspore.Tensor] = None,
...
def compute_mup_vector(config):
    ...
    mup_vector = mint.ones(1, 1, vector_shape)
...
class FalconH1Model(FalconH1PreTrainedModel):
    def __init__(self, config: FalconH1Config):
        ...
        self.layers = mindspore.nn.CellList(decoder_layers)
