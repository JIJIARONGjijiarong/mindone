...
class LightGlueKeypointMatchingOutput(ModelOutput):
    ...
    prune (`mindspore.Tensor` of shape `(batch_size, num_keypoints)`):
    ...
    matches: Optional[ms.Tensor] = None
    matching_scores: Optional[ms.Tensor] = None
    keypoints: Optional[ms.Tensor] = None
    prune: Optional[ms.Tensor] = None
    mask: Optional[ms.Tensor] = None
    hidden_states: Optional[tuple[ms.Tensor]] = None
    attentions: Optional[tuple[ms.Tensor]] = None


class LightGluePositionalEncoder(ms.nn.Cell):
    def __init__(self, config: LightGlueConfig):
        super().__init__()
        self.projector = mint.nn.Linear(2, config.descriptor_dim // config.num_attention_heads // 2, bias=False)

    def construct(
        self, keypoints: ms.Tensor, output_hidden_states: Optional[bool] = False
        ...
        cosines = mint.cos(embeddings)
        sines = mint.sin(embeddings)
...
class LightGlueAttention(ms.nn.Cell):
...
class LightGlueMLP(ms.nn.Cell):
    def __init__(self, config: LightGlueConfig):
        super().__init__()
        self.config = config
        self.activation_fn = ACT2FN[config.hidden_act]
        self.fc1 = mint.nn.Linear(config.intermediate_size, config.intermediate_size)
        self.fc2 = mint.nn.Linear(config.intermediate_size, config.hidden_size)
        self.layer_norm = mint.nn.LayerNorm(config.intermediate_size, elementwise_affine=True)
...
class LightGlueTransformerLayer(ms.nn.Cell):
    def __init__(self, config: LightGlueConfig, layer_idx: int):
        super().__init__()
    ...
    def construct(
        self,
        descriptors: ms.Tensor,
        keypoints: ms.Tensor,
        attention_mask: ms.Tensor,
        output_hidden_states: Optional[bool] = False,
        output_attentions: Optional[bool] = False,
    ) -> tuple[ms.Tensor, Optional[tuple[ms.Tensor]], Optional[tuple[ms.Tensor]]]:
        ...
        intermediate_states = mint.cat([descriptors, attention_output], dim=-1)
        ...
        cross_intermediate_states = mint.cat([self_attention_descriptors, cross_attention_output], dim=-1)
...
def sigmoid_log_double_softmax(similarity: ms.Tensor, matchability0: ms.Tensor, matchability1: ms.Tensor) -> ms.Tensor:
    ...
    scores[:, :-1, -1] = mint.nn.functional.logsigmoid(-matchability0.squeeze(-1))
    scores[:, -1, :-1] = mint.nn.functional.logsigmoid(-matchability1.squeeze(-1))
    return scores


class LightGlueMatchAssignmentLayer(ms.nn.Cell):
    def __init__(self, config: LightGlueConfig):
        super().__init__()

        self.descriptor_dim = config.descriptor_dim
        self.final_projection = mint.nn.Linear(self.descriptor_dim, self.descriptor_dim, bias=True)
        self.matchability = mint.nn.Linear(self.descriptor_dim, 1, bias=True)

    def construct(self, descriptors: ms.Tensor, mask: ms.Tensor) -> ms.Tensor:
        batch_size, num_keypoints, descriptor_dim = descriptors.shape
    ...
    def get_matchability(self, descriptors: ms.Tensor) -> ms.Tensor:
        """Get matchability of descriptors as a probability"""
        matchability = self.matchability(descriptors)
        matchability = mint.nn.functional.sigmoid(matchability).squeeze(-1)
        return matchability


class LightGlueTokenConfidenceLayer(ms.nn.Cell):
    def __init__(self, config: LightGlueConfig):
        super().__init__()

        self.token = mint.nn.Linear(config.descriptor_dim, 1)

    def construct(self, descriptors: ms.Tensor) -> ms.Tensor:
        token = self.token(descriptors.clone())
        token = mint.nn.functional.sigmoid(token).squeeze(-1)
        return token
...
def get_matches_from_scores(scores: ms.Tensor, threshold: float) -> tuple[ms.Tensor, ms.Tensor]:
    ...
    matches0 = mint.where(valid0, matches0, -1)
    matches1 = mint.where(valid1, matches1, -1)
    matches = mint.stack([matches0, matches1]).transpose(0, 1).reshape(batch_size * 2, -1)
    matching_scores = mint.stack([matching_scores0, matching_scores1]).transpose(0, 1).reshape(batch_size * 2, -1)

    return matches, matching_scores


def normalize_keypoints(keypoints: ms.Tensor, height: int, width: int) -> ms.Tensor:
    """
    ...
    """
    size = ms.Tensor([width, height], dtype=keypoints.dtype)[None]
    shift = size / 2
...
class LightGlueForKeypointMatching(LightGluePreTrainedModel):
    ...
    def _keypoint_processing(
        self, descriptors: ms.Tensor, keypoints: ms.Tensor, output_hidden_states: Optional[bool] = False
    ) -> tuple[ms.Tensor, tuple[ms.Tensor, ms.Tensor]]:
        descriptors = descriptors.clone().contiguous()
    ...
    def _get_early_stopped_image_pairs(
        self, keypoint_confidences: ms.Tensor, layer_index: int, mask: ms.Tensor, num_points: ms.Tensor
    ) -> ms.Tensor:
        ...
        if layer_index < self.num_layers - 1:
            ...
            # all pairs of images.
            early_stopped_pairs = mint.ones(batch_size, dtype=ms.bool_)
    ...
    def _get_pruning_mask(self, confidences: ms.Tensor, scores: ms.Tensor, layer_index: int) -> ms.Tensor:
    ...
    def _do_layer_keypoint_pruning(
        ...
        mask: ms.Tensor,
        indices: ms.Tensor,
        prune_output: ms.Tensor,
        keypoint_confidences: ms.Tensor,
        layer_index: int,
    ):
        """
        ...
        pruned_keypoints_mask = pruned_keypoints_mask.masked_fill(mask == 0, ms.Tensor(False))
    ...
    def _concat_early_stopped_outputs(
        ...
        early_stops_indices = mint.stack(early_stops_indices)
    ...
    def _do_final_keypoint_pruning(
        ...
        indices: ms.Tensor,
        matches: ms.Tensor,
        matching_scores: ms.Tensor,
        num_keypoints: ms.Tensor,
        ...
        for i in range(batch_size // 2):
            ...
            _matches[i, 1, indices1[i]] = mint.where(
    ...
    def _match_image_pair(
    ...
    ) -> tuple[ms.Tensor, ms.Tensor, ms.Tensor, tuple, tuple]:
        all_hidden_states = () if output_hidden_states else None
        all_attentions = () if output_attentions else None
        ...
        if keypoints.shape[2] == 0:  # no keypoints
            ...
            return (
                mint.full(shape, -1, dtype=ms.int32),
        ...
        num_points_per_pair = mint.sum(mask.reshape(batch_size, -1), dim=1)
        ...
        for layer_index in range(self.num_layers):
            ...
            if do_early_stop:
                ...
                if mint.any(early_stopped_pairs):
                ...
                if mint.all(early_stopped_pairs):
                    break
        ...
        if do_early_stop and do_keypoint_pruning:
            ...
            final_pruned_keypoints_iterations = mint.ones_like(matching_scores) * self.num_layers
