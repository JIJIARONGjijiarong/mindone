...
_CHECKPOINT_FOR_DOC = "google/mt5-small"
...
# Copied from transformers.models.t5.modeling_t5.T5LayerNorm with T5->MT5
class MT5DenseActDense(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        if (
            isinstance(self.wo.weight, Tensor)
...
class MT5DenseGatedActDense(nn.Cell):
    def __init__(self, config: MT5Config):
        super().__init__()
...
class MT5Attention(nn.Cell):
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        if bidirectional:
            ...
            relative_position = -mint.minimum(relative_position, ops.zeros_like(relative_position))
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if past_key_value is not None:
                if key_value_states is None:
                    ...
                    hidden_states = mint.cat([past_key_value, hidden_states], axis=2)
...
class MT5LayerSelfAttention(nn.Cell):
...
class MT5LayerCrossAttention(nn.Cell):
...
class MT5Block(nn.Cell):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.is_decoder = config.is_decoder
        layer = []
        layer.append(MT5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
            layer.append(MT5LayerCrossAttention(config))
        layer.append(MT5LayerFF(config))
    ...
    def construct(
        ...
        if hidden_states.dtype == ms.float16:
            clamp_value = mint.where(
                mint.any(mint.isinf(hidden_states)),
        ...
        # hidden-states, present_key_value_states, (self-attention position bias), (self-attention weights),
...
class MT5PreTrainedModel(MSPreTrainedModel):
    """
    ...
    def _shift_right(self, input_ids):
        ...
        shifted_input_ids = input_ids.new_zeros(input_ids.shape, dtype=input_ids.dtype)
...
class MT5Model(MT5PreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindone.transformers import MT5Model
...
class MT5ForConditionalGeneration(MT5PreTrainedModel):
    ...
    def construct(
        ...
        >>> tokenizer = AutoTokenizer.from_pretrained("google-mt5/mt5-large")
        >>> model = MT5ForConditionalGeneration.from_pretrained("google-mt5/mt5-large")
        ...
        >>> outputs = model(input_ids=Tensor(input_ids), labels=Tensor(labels, ms.int32))
        >>> loss = outputs[0]
        >>> logits = outputs[1]
        ```"""
        use_cache = use_cache if use_cache is not None else self.use_cache
